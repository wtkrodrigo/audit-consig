...
# --- MÓDULO EMPRESA (COM FILTROS E PERFORMANCE) ---
elif menu == "🏢 Empresa":
    ...
    if not st.session_state.at:
        ...
        if not df_empresa.empty:
            ...
            c_m1.metric("Total de Funcionários", len(df_empresa))
                ...
                try:
                    with st.spinner("Processando..."):
                        ...
                        if payloads:
                            ...
                            st.toast("Sincronizado!", icon="✅"); st.rerun()
                except Exception as e: st.error(f"Erro: {e}")

        with c_act2:
            if not df_empresa.empty:
                st.download_button("📥 EXPORTAR CSV", df_empresa.to_csv(index=False).encode('utf-8'), "auditoria.csv", use_container_width=True)

        with c_act3:
            busca = st.text_input("🔍 Pesquisar", placeholder="Nome ou CPF...", label_visibility="collapsed")
        ...
        if not df_empresa.empty:
            ...
            st.dataframe(df_f, use_container_width=True, hide_index=True, column_config={
                ...
                "data_processamento": st.column_config.DatetimeColumn("Processado em", format="DD/MM/YY HH:mm")
            })

# --- MÓDULO ADMIN MASTER (TODOS OS CAMPOS RESTAURADOS) ---
